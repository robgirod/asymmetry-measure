...
vol_pix = regionprops3(im_bin, "Volume"); % To make sure we get the centroid of the biggest object, if there is noise too
center_of_volume = size(im)/2;

[~, idx] = max(vol_pix.Volume);
transl = center_of_volume - weighted_center_of_mass.WeightedCentroid(idx,:);
...
% Chose "BoundsStyle":
% "CenterOutput"    Keeps the same field of view, will crop if too far
% "FollowOutput"    Pads the image to ensure the rotated particle fits
OutputView = affineOutputView(size(im),tform,"BoundsStyle", "CenterOutput");

aligned_reconstruction = imwarp(reconstruction, tform, "cubic", "OutputView", OutputView);
